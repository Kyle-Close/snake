...
const SnakeSlice = createSlice({
	...
	reducers: {
        ...
        moveSnakeOnUpdate(state, action: PayloadAction<{
            direction: Direction
        }>) {
            const snake = state.snake
            ...
            for(let i = 0; i < snakeLength; i++){
                if(i === 0){
                    ...
                    snake[i].next = getNextHeadCoordinates(action.payload.direction, snake[i].current)
                }else{
                    // On body
                    snake[i].current = snake[i].next
...
export const { initSnakeHead, addSnakeBodySegment, moveSnakeOnUpdate } = SnakeSlice.actions;
